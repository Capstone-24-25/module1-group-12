...
We first mention the use of the **correlation** approach as our first method, which collects the correlation values between each protein and the ADOS (Autism Diagnostic Observation Schedule) - which is the measure of ASD symptom severity. To determine the most important relationships, the 10 largest values in magnitude are then selected to be the 'pre-panel' of predictive proteins. Next approach is using **t-tests** to determine the significance between each protein for each group. This is done by finding the sample mean of the protein values for each group and comparing to see if the sample means are indifferent. Essentially, we are testing if $\bar\mu_{TD_{1}} - \bar\mu_{ASD_{1}} = 0$ for an arbitrary protein, for each group. We can discover the most significant values by evaluating the p-values for each test. Those with the lowest values are accepted to be the most critical for our panel from the t-test approach. Lastly, we use a **random forest** model which not only delivers a model for testing, but can help determine feature (or variable) importance, which is ideal for our situation of selecting the most important proteins. After performing each model, we collected the top predictive proteins from all three methods, and used a 'fuzzy' intersection along with a 'hard' to find what all methods find to be important, denoting these as 'core' proteins. After collecting the 'core' proteins, we then run a logistic regression model with a classification setting to output a group. The results for both intersections are displayed below.

The '**fuzzy'** intersection of the 3 methods posed a panel consisting of:

**Calcineurin, MAPK2, (CXCL16, soluble), DERM, RELT, IGFBP-1, MAPK14, ALCAM, Notch 1, Fas, soluble, ERBB1, IgD, eIF-4H, MMP-2.**

Using a '**hard**' intersection with the R function `instersect`, we have the following group of proteins:

**MAPK2, DERM, RELT, IGFBP-1, MAPK14, ALCAM, Notch 1, (gp130, soluble), IgD, PPID, Coagulation Factor IX.**

The first intersection had an estimated accuracy of \~ 67.7% on the testing set, and the hard interestion had an estimated accuracy of \~ 70.9% on the testing set.
...
When using a fuzzy intersection, the accuracy increased slightly, with a score of about 77.4%, when compared to the prior panel. This indicates that the fuzzy intersection panel can classify ASD correctly only about 77.4% of the time on the testing set.
...
When using a hard intersection, the accuracy increased as well, with a score of about 70.9% when compared to the prior panel. This indicates that the hard intersection panel can classify ASD correctly only about 70.9% of the time on the testing set.

### Improved classifier

After running a logistic regression on the prior, fuzzy intersection, and hard intersection panels, we improved the accuracy of the prior analysis by about 10% with the hard intersection. The panel is the following.
...
We have also achieved similar accuracy to the prior, at 70.9% (compared to 67.7%) with a fuzzy intersection. The panel is the following.
