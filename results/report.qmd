...
We first mention the use of the **correlation** approach as our first method, which collects the correlation values between each protein and the ADOS (Autism Diagnostic Observation Schedule) - which is the measure of ASD symptom severity. To determine the most important relationships, the 10 highest values in magnitude are then selected to be the "pre-panel" of predictive proteins.

Next, we conducted **t-tests** to compare the mean protein levels between ASD and TD groups. This test evaluates whether the sample means differ significantly, testing the hypothesis that $\bar\mu_{TD_{i}} - \bar\mu_{ASD_{i}} = 0$ for an arbitrary protein, $i$, for each group. Proteins with the lowest p-values (indicating significant differences) were deemed the most critical for inclusion in the t-test-based panel.

Lastly, we use a **random forest** model which not only delivers a model for testing, but can help determine feature (or variable) importance, which is ideal for our situation of selecting the most important proteins.

After performing all three methods, we combined the results by identifying top predictive proteins from each approach. We used both a “fuzzy” and a “hard” intersection to find consensus proteins across methods, designating these as “core” proteins. Finally, a logistic regression model was applied to this core panel to classify subjects.

The “fuzzy” intersection panel consisted of:\
*Calcineurin, MAPK2, CXCL16 (soluble), DERM, RELT, IGFBP-1, MAPK14, ALCAM, Notch 1, Fas (soluble), ERBB1, IgD, eIF-4H, MMP-2.*

The “hard” intersection (using the R function `intersect`) included:\
*MAPK2, DERM, RELT, IGFBP-1, MAPK14, ALCAM, Notch 1, gp130 (soluble), IgD, PPID, Coagulation Factor IX.*

**Performance**\
The fuzzy intersection achieved an estimated accuracy of 67.7% on the testing set, while the hard intersection reached 70.9%.
...
When we visualise this with a box plot, we see there is a denser set of outliers in the TD class compared to the ASD class, which implies that there are more subjects in TD with a high amount of outliers compared to ASD.  
...
After running a logistic regression on the prior, fuzzy intersection, and hard intersection panels on the training set and testing the models, we improved the accuracy of the prior analysis by about 10% with the hard intersection. The protein panel for this approach is the following:
...
We have also achieved similar accuracy to the prior, at 70.9% (compared to 67.7%) with a fuzzy intersection. The protein panel is as follows:
