...
From there, we wanted to take a look at which group these individuals with outliers were coming from. To analyze this, we worked with protein values that were equal to 3 or -3, since any outliers in the data were trimmed to those values. After visualizing the outlier count per group (ASD, TD) in a box plot and tabling the data, we see there is a total of 75 ASD subjects with at least one outlier and 77 subjects in the TD group. However, when we visualize this with a box plot, we see there is a denser set of outliers in the TD class compared to the ASD class, which implies that there are more subjects in TD with a high amount of outliers compared to ASD. TD also has a higher amount of total outliers as a group (ASD 1007, TD 1372). When displaying the top 10 most outliers, 7 of the top 10 are from the TD group and 3 are from the ASD group, with the top 2 spots from the TD group. In conclusion, while many subjects have at least 1 outlier, the subjects with a very high amount of outliers mostly belong to the TD group and the subject with the most outliers is in the TD group.
...
biomarker_sstar <- biomarker_clean %>%
  select(group, any_of(proteins_sstar_hard)) %>%
